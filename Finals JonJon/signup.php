...
function generateStudentNumber($conn) {
    ...
    $query = "SELECT student_number FROM student WHERE student_number LIKE '{$year}-%' ORDER BY student_number DESC LIMIT 1";
    $result = $conn->query($query);
    
    if ($result->num_rows > 0) {
        $last_number = $result->fetch_assoc()['student_number'];
        $last_sequence = intval(substr($last_number, 5));
...
function generateTeacherCode($conn) {
    ...
    // Get the highest teacher code for this year
    $query = "SELECT teacher_code FROM teacher WHERE teacher_code LIKE '{$prefix}%' ORDER BY teacher_code DESC LIMIT 1";
    $result = $conn->query($query);
    
    if ($result->num_rows > 0) {
        $last_code = $result->fetch_assoc()['teacher_code'];
        $last_sequence = intval(substr($last_code, 10));
        $new_sequence = $last_sequence + 1;
    } else {
        $new_sequence = 1;
    }
...
$generated_teacher_code = '';

// Generate student number if requested
if (isset($_GET['generate_student'])) {
    $generated_student_number = generateStudentNumber($conn);
}

// Generate teacher code if requested
if (isset($_GET['generate_teacher'])) {
    $generated_teacher_code = generateTeacherCode($conn);
}
...
<body>
          ...
          <div class="alt-input-group">
            ...
            <div class="input-with-button">
              <input type="text" id="snumber" name="snumber" 
                     placeholder="Enter Student Number or Generate" 
                     value="<?php echo isset($_POST['snumber']) ? htmlspecialchars($_POST['snumber']) : (isset($generated_student_number) ? htmlspecialchars($generated_student_number) : ''); ?>">
              <button type="button" class="generate-btn" onclick="generateStudentNumber()">🔄 Generate</button>
            </div>
          </div>
          ...
          <div class="alt-input-group">
            ...
            <div class="input-with-button">
              <input type="text" id="teacher_code" name="teacher_code" 
                     placeholder="Enter Teacher Code or Generate" 
                     value="<?php echo isset($_POST['teacher_code']) ? htmlspecialchars($_POST['teacher_code']) : (isset($generated_teacher_code) ? htmlspecialchars($generated_teacher_code) : ''); ?>">
              <button type="button" class="generate-btn" onclick="generateTeacherCode()">🔄 Generate</button>
            </div>
          </div>
  ...
  <script>
    ...
    function generateStudentNumber() {
      fetch('signup.php?generate_student=1')
        .then(response => response.text())
        .then(html => {
          // Extract the generated student number from the response
          const tempDiv = document.createElement('div');
          tempDiv.innerHTML = html;
          const inputField = tempDiv.querySelector('#snumber');
          if (inputField && inputField.value) {
            document.getElementById('snumber').value = inputField.value;
          } else {
            // Fallback: Generate client-side
            const year = new Date().getFullYear();
            const randomNum = Math.floor(Math.random() * 9000) + 1000;
            document.getElementById('snumber').value = year + '-' + randomNum;
          }
        })
        .catch(() => {
          // Fallback: Generate client-side if fetch fails
          const year = new Date().getFullYear();
          const randomNum = Math.floor(Math.random() * 9000) + 1000;
          document.getElementById('snumber').value = year + '-' + randomNum;
        });
    ...
    function generateTeacherCode() {
      fetch('signup.php?generate_teacher=1')
        .then(response => response.text())
        .then(html => {
          const tempDiv = document.createElement('div');
          tempDiv.innerHTML = html;
          const inputField = tempDiv.querySelector('#teacher_code');
          if (inputField && inputField.value) {
            document.getElementById('teacher_code').value = inputField.value;
          } else {
            // Fallback: Generate client-side
            const year = new Date().getFullYear();
            const randomNum = Math.floor(Math.random() * 9000) + 1000;
            document.getElementById('teacher_code').value = 'TCH' + year + randomNum;
          }
        })
        .catch(() => {
          // Fallback: Generate client-side if fetch fails
